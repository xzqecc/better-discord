...
local ra = syn.crypt.hash(ip)

if ra == "92f909345747f45870518d075e8376fba004a101210a558058720d29099218ce3f069c853e2af45b062ffc5a519f347d" then
	BBOT = { username = "dev" }
else
	BBOT = { username = "User" }
end
...
local MenuName = isfile("xzs/menuname.txt") and readfile("xzs/menuname.txt") or nil
...
if not isfile("xzs/chatspam.txt") then --idk help the user out lol, prevent stupid errors --well it would kinda ig
	writefile("xzs/chatspam.txt", [[]])
end

if not isfile("xzs/killsay.txt") then
	writefile("xzs/killsay.txt", [[]])
end
if not isfile("xzs/chatspam.txt") then
	writefile("xzs/chatspam.txt", [[]])
end
...
do
	...
	CreateNotification =
		function(t, customcolor) -- TODO i want some kind of prioritized message to the notification list, like a warning or something. warnings have icons too maybe? idk??
					...
					if self.enabled then
						...
						for i, drawing in pairs(self.drawings) do
							...
							elseif i:find("line") then
								...
								if menu then
									local mencol = customcolor
										or (
											menu:GetVal("Settings", "Cheat Settings", "Menu Accent")
												and Color3.fromRGB(
													unpack(
														menu:GetVal("Settings", "Cheat Settings", "Menu Accent", COLOR)
													)
												)
											or Color3.fromRGB(127, 72, 163)
										)
									local color = linenum == 1 and mencol
										or Color3.fromRGB(mencol.R * 255 - 40, mencol.G * 255 - 40, mencol.B * 255 - 40) -- super shit
						...
						time = time + estep * dt * 128 -- TODO need to do the duration
			...
			local color = (menu and menu.GetVal) and customcolor
				or menu:GetVal("Settings", "Cheat Settings", "Menu Accent") and Color3.fromRGB(
					unpack(menu:GetVal("Settings", "Cheat Settings", "Menu Accent", COLOR))
				)
				or Color3.fromRGB(127, 72, 163)
...
if not isfolder("xzs") then
	makefolder("xzs")
	if not isfile("xzs/relations.xz") then
		writefile("xzs/relations.xz", "bb:{{friends:}{priority:}")
	end
else
	if not isfile("xzs/relations.xz") then
		writefile("xzs/relations.xz", "bb:{{friends:}{priority:}")
	end
	writefile("xzs/debuglog.xz", "")
end

if not isfolder("xzs/" .. menu.game) then
	makefolder("xzs/" .. menu.game)
end
...
local function GetConfigs()
	...
	if #result <= 0 then
		writefile("xzs/" .. menu.game .. "/Default.xz", "")
	end
	return result
end
...
local function UnpackRelations()
	local str = isfile("xzs/relations.xz") and readfile("xzs/relations.xz") or nil
	local final = {
		friends = {},
		priority = {},
	}
	if str then
		if str:find("bb:{{") then
			writefile("xzs/relations.xz", "friends:\npriority:")
			return
		end
...
local function WriteRelations()
	...
	writefile("xzs/relations.xz", str)
end
CreateThread(function()
...
do
	...
	function Draw:CoolMultiBox(names, x, y, width, height, tab)
		...
		for i, v in ipairs(names) do
			...
			length = length + tab[#tab - 1].TextBounds.X + 10
		end
...
function menu.Initialize(menutable)
	...
	do
		...
		Draw:MenuBigText(getgenv().menuname, true, false, 6, 6, bbmenu)
					...
					if content ~= nil then
						for k2, v2 in pairs(content) do
							if v2.type == TOGGLE then
								...
								menu.options[v.name][g_name][v2.name][4] = Draw:Toggle(
									v2.name,
									v2.value,
									unsafe,
									v1.x + 8,
								...
								if v2.extra ~= nil then
									if v2.extra.type == KEYBIND then
										...
										menu.options[v.name][g_name][v2.name][5][3] = {
											v1.x + v1.width - 52,
											y_pos + v1.y - 2,
										}
										menu.options[v.name][g_name][v2.name][5][5] = false
										menu.options[v.name][g_name][v2.name][5].toggletype = v2.extra.toggletype == nil
												and 1
											or v2.extra.toggletype
										...
										event:connect(function(newval)
											if menu:GetVal("Visuals", "Keybinds", "Log Keybinds") then
												CreateNotification(
													("%s %s %s has been set to %s"):format(
														v.name,
														g_name,
														v2.name,
														newval and "true" or "false"
													)
												)
											end
										end)
									...
									elseif v2.extra.type == DOUBLE_COLORPICKER then
										...
										for i = 1, 2 do
											...
											menu.options[v.name][g_name][v2.name][5][1][i][3] = {
												v1.x + v1.width - 38 - ((i - 1) * 34),
												y_pos + v1.y - 1,
											}
							...
							elseif v2.type == SLIDER then
								...
								menu.options[v.name][g_name][v2.name][7] = {
									v1.x + 7 + v1.width - 38,
									v1.y + y_pos - 1,
								}
							...
							elseif v2.type == DROPBOX then
								...
								if v2.x == nil then
									menu.options[v.name][g_name][v2.name][3] = {
										v1.x + 7,
										v1.y + y_pos - 1,
										v1.width - 16,
									}
							...
							elseif v2.type == BUTTON then
								...
								if v2.x == nil then
									...
									menu.options[v.name][g_name][v2.name][4] = Draw:Button(
										v2.name,
										v1.x + 8,
										v1.y + y_pos,
										v1.width - 16,
							...
							elseif v2.type == TEXTBOX then
								menu.options[v.name][g_name][v2.name] = {}
								menu.options[v.name][g_name][v2.name][4] = Draw:TextBox(
									v2.name,
									v2.text,
									v1.x + 8,
									v1.y + y_pos,
							...
							elseif v2.type == "list" then
								...
								y_pos = y_pos + 22 + (22 * v2.size)
							elseif v2.type == IMAGE then
								menu.options[v.name][g_name][v2.name] = {}
								menu.options[v.name][g_name][v2.name][1] = Draw:ImageWithText(
									v2.size,
									nil,
									v2.text,
									v1.x + 8,
	...
	function menu:MouseInMenu(x, y, width, height)
		return LOCAL_MOUSE.x > menu.x + x
			and LOCAL_MOUSE.x < menu.x + x + width
			and LOCAL_MOUSE.y > menu.y - 36 + y
			and LOCAL_MOUSE.y < menu.y - 36 + y + height
	end

	function menu:MouseInColorPicker(x, y, width, height)
		return LOCAL_MOUSE.x > cp.x + x
			and LOCAL_MOUSE.x < cp.x + x + width
			and LOCAL_MOUSE.y > cp.y - 36 + y
			and LOCAL_MOUSE.y < cp.y - 36 + y + height
	end
			...
			for k1, v1 in pairs(menu.options) do
				for k2, v2 in pairs(v1) do
					for k3, v3 in pairs(v2) do
						if v3[2] == k and k3 ~= "Configs" and k3 ~= "Player Status" and k3 ~= "ConfigName" then
	...
	local function LoadConfig(loadedcfg)
		...
		if lines[1] == "xzs v2" then
			...
			for i = 1, end_ - start - 1 do
				...
				if
					menu.options[tt[1]] ~= nil
					and menu.options[tt[1]][tt[2]] ~= nil
					and menu.options[tt[1]][tt[2]][tt[3]] ~= nil
				then
					if tt[4] == "true" then
			...
			for i = 1, end_ - start - 1 do
				...
				if
					...
					and menu.options[tt[1]][tt[2]] ~= nil
					and menu.options[tt[1]][tt[2]][tt[3]] ~= nil
					and menu.options[tt[1]][tt[2]][tt[3]][5] ~= nil
				then
					if tt[5] ~= nil then
				...
				for k1, v1 in pairs(v) do
					for k2, v2 in pairs(v1) do
						...
						elseif v2[2] == SLIDER then
							...
							v2[4][5].Text = v2.custom[v2[1]]
								or (v2[1] == math.floor(v2[1]) and v2.decimal) and tostring(v2[1]) .. "." .. decplaces .. v2[4][6]
								or tostring(v2[1]) .. v2[4][6]
	...
	function menu.saveconfig()
		...
		writefile(
			"xzs/" .. menu.game .. "/" .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. ".xz",
			figgy
		)
		CreateNotification('Saved "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.xz"!')
		UpdateConfigs()
	end

	function menu.loadconfig()
		local configname = "xzs/"
			.. menu.game
			.. "/"
		...
		if not isfile(configname) then
			CreateNotification(
				'"' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.xz" is not a valid config.'
			)
			return
		end
		...
		if pcall(LoadConfig, loadedcfg) then
			CreateNotification('Loaded "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.xz"!')
		else
			LoadConfig(curcfg)
			CreateNotification(
				'There was an issue loading "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.xz"'
			)
		end
	end
	...
	local function buttonpressed(bp)
		...
		if bp == menu.options["Settings"]["Cheat Settings"]["Unload Cheat"] then
			...
			wait()
			menu:unload()
		elseif bp == menu.options["Settings"]["Cheat Settings"]["Set Clipboard Job ID"] then
			setclipboard(game.JobId)
			CreateNotification("Set Clipboard Job ID! (" .. tostring(game.JobId) .. ")")
		elseif bp == menu.options["Settings"]["Cheat Settings"]["Set Clipboard Place ID"] then
			setclipboard(game.PlaceId)
			CreateNotification("Set Clipboard Place ID! (" .. tostring(game.PlaceId) .. ")")
		...
		elseif bp == menu.options["Settings"]["Configuration"]["Delete Config"] then
			delfile("xzs/" .. menu.game .. "/" .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. ".xz")
			CreateNotification('Deleted "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.xz"!')
			UpdateConfigs()
							...
							for k2, v2 in pairs(v1) do
								if v2[2] == TOGGLE and not menu.dropbox_open then
									if menu:MouseInMenu(v2[3][1], v2[3][2], 30 + v2[4][5].TextBounds.x, 16) then
										if v2[6] then
											if menu:GetVal("Settings", "Cheat Settings", "") and v2[1] == false then
												v2[1] = true
											else
												v2[1] = false
											end
								...
								elseif v2[2] == SLIDER and not menu.dropbox_open then
									if menu:MouseInMenu(v2[7][1], v2[7][2], 22, 13) then
										...
										local decplaces = v2.decimal
											and string.rep("0", math.log(1 / v2.decimal) / math.log(10))
										if decplaces and math.abs(v2[1]) < v2.decimal then
											v2[1] = 0
										end
										v2[4][5].Text = v2.custom[v2[1]]
								...
								elseif v2[2] == DROPBOX then
									...
									elseif
										menu:MouseInMenu(v2[3][1], v2[3][2], v2[3][3], 24 * (#v2[6] + 1) + 3) and v2[5]
									then
										for i = 1, #v2[6] do
											if
												menu:MouseInMenu(v2[3][1], v2[3][2] + 36 + ((i - 1) * 21), v2[3][3], 21)
											then
												v2[4][1].Text = v2[6][i]
												v2[1] = i
												menu:SetDropBox(false, 400, 200, 160, 1, { "HI q", "HI q", "HI q" })
										...
										if v2 == menu.options["Settings"]["Configuration"]["Configs"] then
											...
											textbox[1] =
												relconfigs[menu.options["Settings"]["Configuration"]["Configs"][1]]
								...
								elseif v2[2] == COMBOBOX then
									...
									elseif
										menu:MouseInMenu(v2[3][1], v2[3][2], v2[3][3], 24 * (#v2[1] + 1) + 3) and v2[5]
									then
										for i = 1, #v2[1] do
											if
												menu:MouseInMenu(v2[3][1], v2[3][2] + 36 + ((i - 1) * 22), v2[3][3], 23)
											then
		...
		for k, v in pairs(menu.options) do
			for k1, v1 in pairs(v) do
				for k2, v2 in pairs(v1) do
					if v2[2] == TOGGLE then
						if v2[6] then
							if not menu:GetVal("Settings", "Cheat Settings", "") then
								v2[1] = false
								for i = 0, 3 do
									v2[4][i + 1].Color = ColorRange(i, {
										[1] = { start = 0, color = RGB(50, 50, 50) },
	...
	local function renderSteppedMenu(fdt)
		...
		if menu.open then
			if menu.backspaceheld then
				...
				if dt > 0.4 then
					menu.backspaceflags = menu.backspaceflags + 1
		...
		if menu.open or menu.fading then
							...
							for k2, v2 in pairs(v1) do
								if v2[2] == TOGGLE then
									if not menu.dropbox_open and not menu.colorPickerOpen then
										if
											menu.open
											and menu:MouseInMenu(v2[3][1], v2[3][2], 30 + v2[4][5].TextBounds.x, 16)
										then
								...
								elseif v2[2] == SLIDER then
									if v2[5] then
										local new_val = (v2[6][2] - v2[6][1])
											* ((LOCAL_MOUSE.x - menu.x - v2[3][1]) / v2[3][3])
										v2[1] = (
												not v2.decimal and math.floor(new_val)
											...
											for i1 = 1, v2[7] do
												...
												if v2[5][i + v2[3] - 1][i1][1] == v2[1] and i1 == 1 then
													if menu.options["Settings"]["Cheat Settings"]["Menu Accent"][1] then
														local clr =
															menu.options["Settings"]["Cheat Settings"]["Menu Accent"][5][1]
														v2[4].liststuff.words[i][i1].Color = RGB(clr[1], clr[2], clr[3])
													else
														v2[4].liststuff.words[i][i1].Color = RGB(
															menu.mc[1],
															menu.mc[2],
															menu.mc[3]
														)
			...
			menu.inmenu = LOCAL_MOUSE.x > menu.x
				and LOCAL_MOUSE.x < menu.x + menu.w
				and LOCAL_MOUSE.y > menu.y - 32
				and LOCAL_MOUSE.y < menu.y + menu.h - 34
			menu.intabs = LOCAL_MOUSE.x > menu.x + 9
				and LOCAL_MOUSE.x < menu.x + menu.w - 9
				and LOCAL_MOUSE.y > menu.y - 9
				and LOCAL_MOUSE.y < menu.y + 24
			menu.inmiddlemenu = LOCAL_MOUSE.x > menu.x + 18
				and LOCAL_MOUSE.x < menu.x + menu.w - 18
				and LOCAL_MOUSE.y > menu.y + 33
				and LOCAL_MOUSE.y < menu.y + menu.h - 56
			if
				(
										--[[(
						LOCAL_MOUSE.x > menu.x and LOCAL_MOUSE.x < menu.x + menu.w and LOCAL_MOUSE.y > menu.y - 32 and LOCAL_MOUSE.y < menu.y - 11
					)]]
(menu.inmenu and not menu.intabs and not menu.inmiddlemenu) or menu.dragging
		...
		if menu then
			...
			if menu.open then
				if menu.tabnames[menu.activetab] == "Settings" then
					local menutext = getgenv().menuname
...
menu.connections.heartbeatmenu = game.RunService.Heartbeat:Connect(function() --ANCHOR MENU HEARTBEAT
	...
	if tick() > lasttick + 0.25 then
		...
		graphs.incoming.sides[2].Position = Vector2.new(
			graphs.incoming.pos.x - ((#numstr + 1) * 7),
			graphs.incoming.pos.y - 7
		)
		...
		local avg_color = menu:GetVal("Settings", "Cheat Settings", "Menu Accent") and RGB(unpack(menu.mc))
			or RGB(59, 214, 28)
		...
		graphs.outgoing.sides[2].Position = Vector2.new(
			graphs.outgoing.pos.x - ((#numstr + 1) * 7),
			graphs.outgoing.pos.y - 7
		)
		...
		if menu.open then
			if menu.textboxopen then
				...
				if
					...
					and inputObject.KeyCode ~= Enum.KeyCode.Delete
				then
					if menu.selectall then
						menu.textboxopen[4].Color = RGB(unpack(menu.mc))
						menu.selectall = false
				...
				if inputObject.KeyCode == Enum.KeyCode.A then
					if
						inputState == Enum.UserInputState.Begin and INPUT_SERVICE:IsKeyDown(Enum.KeyCode.LeftControl)
					then
						menu.selectall = true
						local textbox = menu.textboxopen
...
if menu.game == "uni" then --SECTION UNIVERSAL
	...
	for i = 1, Players.MaxPlayers do
		...
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 255, 255, 255, 255 }, { 0, 0, 0 }, allesp.talents)
